...
def ensure_requirements(*deps: str):
    ...
    if len(paths) < 2:
        ...
        config.log.warning("Not installing dependencies")
        return
    ...
    try:
        ...
        for specifier, parsed in parsed_deps.items():
            if not parsed:
                config.log.warning(f"Ignoring invalid package specifier: {specifier}")
                continue
        ...
        config.log.debug(f"Failed to import '{e.name}'")

    package_names = [parsed[0] for parsed in parsed_deps.values() if parsed]
    config.log.warning(f"Installing dependencies: {', '.join(package_names)}")
    config.log.warning(
    ...
    config.log.warning(f"Dependencies installed successfully into {deps_folder}")
